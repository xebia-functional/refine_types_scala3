 ...
 *
 * Type aliases in Scala allow you to give alternative names to existing types. They are declared using the 'type' keyword.
 *
 * Basic Syntax:
 * {{{
 ...
 *
 * =Practical Impact=
 * As an example of limitation, let's look at Age and Count:
 * {{{
 * type Age = Int
 * type Count = Int
 * }}}
 * Both Age and Count are just Int, so you can mix them without compiler errors
 */
